...
public:
    vector<int> maxActiveSectionsAfterTrade(string s, vector<vector<int>>& queries) {
        ...
        vector<int> typeList, startList, endList;
        ...
        for (auto &q : queries) {
            ...
            maxGain = max(maxGain,
                evaluateEdge(segL + 1, L, R, segL, segR,
                             type, start, endIdx));

            maxGain = max(maxGain,
                evaluateEdge(segR - 1, L, R, segL, segR,
                             type, start, endIdx));
